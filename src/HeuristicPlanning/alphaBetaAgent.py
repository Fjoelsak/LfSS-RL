...
class alphaBetaAgent(minimaxAgent):
    ...
    def alphabeta(self, obs, is_maximizing, current_player, alpha, beta):
        """
            ...
            for row in range(3):
                for col in range(3):
                    if obs[row, col, 0] == 0 and obs[row, col, 1] == 0:  # empty cell
                        ...
                        value = self.alphabeta(obs, False, 1 - current_player, alpha, beta)
            ...
            for row in range(3):
                for col in range(3):
                    if obs[row, col, 0] == 0 and obs[row, col, 1] == 0:  # empty cell
                        # simulate the move
                        obs[row, col, current_player] = 1
                        value = self.alphabeta(obs, True, 1 - current_player, alpha, beta)
                        obs[row, col, current_player] = 0  # undo move
            ...
            for row in range(3):
                for col in range(3):
                    if obs[row, col, 0] == 0 and obs[row, col, 1] == 0:  # empty cell
                        ...
                        move_value = self.alphabeta(obs, False, 1 - self.player, alpha, beta)
